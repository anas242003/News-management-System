...
adminWidget::adminWidget(QString name ,loadAndStoreFiles* tools,QWidget *parent)
    : QWidget(parent)
{
    tool = tools;
    tool->loadNews();
    ...
    QPixmap image("K:\\C++\\GUI Qt\\NewsGUI\\AdminFrame.png");
    ...
    connect(logout, &QPushButton::clicked, this, [=]()
        {
            ...
            this->hide();
            LoginPage* log = new LoginPage(tool);
            log->show();
        });
...
void adminWidget::makeAddOrUpdateFrame(int id) {
    ...
    connect(confirmBtn, &QPushButton::clicked, this, [=]() {
        ...
        if (updateCat) {
            ...
            remove(tool->categorys[article.category].begin(),
                tool->categorys[article.category].end(),
                id);

            tool->categorys[article.category].push_back(id);
         }
        }
...
void adminWidget::showAddCategoryDialog()
{
    QInputDialog dialog(this);
    dialog.setInputMode(QInputDialog::TextInput);
    dialog.setWindowTitle("New Category");
    dialog.setLabelText("Enter the name of the new category:");
    QLineEdit* lineEdit = dialog.findChild<QLineEdit*>();

    if (lineEdit) {
        lineEdit->setMaxLength(20); // Set the maximum length to 10 characters
    }
    if (dialog.exec() == QDialog::Accepted) {  //check if he press ok button
        QString newCategoryName = dialog.textValue();
        if (!newCategoryName.isEmpty()) {
            // Add the new category to your news management system
            // ...
        }
    }
}
...
void adminWidget::printnewsforadmin(NewsArticle article)
{
    ...
    title->setText(((std::string)article.title).c_str());
    newsboxlayout->addWidget(title, 0, 0, 1, 6);


    string newsPropStr = "Author: " + article.author + " - " +
        "Date: " + article.date + " - " + "Rate: " + article.printRate() + " - " +
        "Category: " + article.category + " - " + "Spam: " + to_string(article.spam);
    ...
    newsboxlayout->addWidget(newsProp, 1, 0, 1, 6);
    ...
    newsboxlayout->addWidget(description, 2, 0, 10, 6);

    QPushButton* deleteBtn = new QPushButton("Delete");
    deleteBtn->setObjectName("deleteBtn");
    deleteBtn->setFixedSize(100, 50);
    deleteBtn->setStyleSheet(
        "#deleteBtn{font-size:20px; font-family:Inter; border-radius: 20px; background-color: #D9D9D9; padding:10px; }\
             #deleteBtn::hover{background-color: #245B96; color:#fff;}"
    );
    newsboxlayout->addWidget(deleteBtn, 12, 0);
    ...
    newsboxlayout->addWidget(updateBtn, 12, 1);
    ...
    newsboxlayout->addWidget(commBg, 13, 0, 8, 6);
    ...
    newsboxlayout->addWidget(commentsLbl, 13, 0);
    ...
    newsboxlayout->addWidget(comments, 14, 0, 8, 6);
