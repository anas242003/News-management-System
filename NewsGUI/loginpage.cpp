...
LoginPage::LoginPage(loadAndStoreFiles* tools, bool fromMain, QWidget *parent) :QWidget{parent}, ui(new Ui::Widget)
{
    ui->setupUi(this);
...
void LoginPage::login() {
    ...
    if (usernameSTD.empty() || passwordSTD.empty()) {
        ...
        if (tool->users[usernameSTD].role == "user") {
            UserWidget* user = new UserWidget(username->text(), tool);
            user->show();
        }
        else {
            adminWidget* admin = new adminWidget(username->text(), tool);
            admin->show();
        }
