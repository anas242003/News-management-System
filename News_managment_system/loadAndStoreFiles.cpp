...
void loadAndStoreFiles::saveAndClose(unordered_map<string, userData > users,unordered_map<int, NewsArticle> news) {
...
void loadAndStoreFiles::saveUsers(unordered_map<string, userData > users) {
    ...
    // add file data with respect to header.
    // explian: 
    //  <user> is paire <name, vector<string>{password, role}>
    //  to get username = <user>.first 
    //         password = <user>.second[0]
    //             role = <user>.second[1]
...
vector<int> loadAndStoreFiles::getSpamVec(string str) {
...
void loadAndStoreFiles::printNewsByRate(unordered_map<int, NewsArticle> news) {
    vector<int> sortedIds = getSortedIds_byRate(news);

    for (auto id : sortedIds)
        printNew(news[id]);
}

void loadAndStoreFiles::printUsers(unordered_map<string, userData > users) {

    for (auto user : users)
    {
        cout << "name : " << user.first << endl;
        cout << "pass : " << user.second.pass << endl;
        cout << "role : " << user.second.role << endl;
        cout << "spam : " << user.second.printSpam() << endl;
        cout << "Prefrence Category : " << user.second.printPrefrenceCategory() << endl;
        cout << endl;
    }
}

void loadAndStoreFiles::printNews(unordered_map<int, NewsArticle> news) {

    cout << "\nAll News: \n";
    for (auto currentNew : news)
        printNew(currentNew.second);

}

void loadAndStoreFiles::printNew(NewsArticle article) {

    cout << "\n" << "news ID  : " << article.id << endl
        << "category : " << article.category << endl
        << "Title    : " << article.title << endl
        << "Author   : " << article.author << endl
        << "Content  : " << article.description << endl
        << "Date     : " << article.date << endl
        << "Rate     : " << article.printRate() << endl
        << "Spam     : " << article.spam << endl
        << "Comment  : " << article.getcomment() << endl << endl;
}